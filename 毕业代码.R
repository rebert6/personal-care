...
PIPHF$variable <- c('2,5-dichlorophenol','2,4-dichlorophenol','BPA','benzophenone-3','TCS','Mep','PrP')
#######
...
pred.resp.univar <- mutate_at(pred.resp.univar,vars('variable'),
                              ~case_when(.=='URX14D'~'2,5-dichlorophenol',
                                         .=='URXDCB'~'2,4-dichlorophenol',
                                         .=='URXBPH'~'BPA',
                                         .=='URXBP3'~'benzophenone-3',
                                         .=='URXTRS'~'TCS',
                                         .=='URXMPB'~'Mep',
                                         .=='URXPPB'~'PrP'))
...
PIPLF$variable <- c('2,5-dichlorophenol','2,4-dichlorophenol','BPA','benzophenone-3','TCS','Mep','PrP')
###????
...
rownames(mean_ra) <- paste0('cluster',1:4)
mean_ra <- rbind(rep(-1.05,7),mean_ra)
mean_ra <- rbind(rep(1.8,7),mean_ra)
colnames(mean_ra) <- c('2,5-dichlorophenol','2,4-dichlorophenol','BPA','benzophenone-3','TCS','Mep','PrP')
jpeg('图7.jpg',width=15000,height=8500,res=600)
par(mfrow=c(1,2),cex.main=2.5)
scatterplot3d(tsnedata,color = color,angle = 72,pch=16,
...
legend("bottomleft",legend = levels(data$cluster),col = color.lib,pch = 16,
       inset = -0.04,xpd = T,horiz = T,cex = 2.5,title = 'cluster',text.col='black')
dev.off()
############
